...
This creates a forward-filled price panel for the target, a global volatility proxy (VIX, `^VIX`), Gold (`GC=F`), and an international bond proxy (`BNDX`), plus daily log-returns for the target, gold, and bonds.
...
- **Sentiment / macro** (`sentiment_macro.py` / `feature_engineer.py`):
  - Implied-vol index daily log-change (`vstoxx_chg`), computed from the volatility proxy (VIX).
  - Vol-of-vol on implied-vol log-returns (`vol_of_vol`), based on a 66-day rolling std lagged one day.
...
You can also run the full feature-building pipeline (including labels and basic health checks) via:

```bash
python -m scripts.build_features
```

which writes `data/processed/features.csv`.

### Diagnostics and stationarity checks

Use `src/diagnostics.py` to audit look-ahead risks and stationarity:

- **ADF tests + differencing + look-ahead scan** (after saving `data/processed/features.csv`):
...
- **Stationary at 5%**: `entropy_66d`, `vol_scaled_return_126d`, `skewness_126d`, `vol_of_vol`, `bollinger_pct_b_20d`, `cmci_20d`, `stoch_k_14d`, `rsi_14d`, `williams_r_14d`, `gold_log_return`, `bund_rets`.
- **Non-stationary, differenced**: `momentum_63d`, `momentum_252d`, `vstoxx_chg` (used `%Δ` versions in models).
...
```bash
python -m scripts.prepare_tensors
```
